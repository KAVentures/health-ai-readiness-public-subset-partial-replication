#!/usr/bin/env python3
"""
MedGemma Benchmark Evaluation - OmniMedVQA

Evaluates MedGemma on the OmniMedVQA dataset.
"""
...
def main():
    ...
    os.makedirs(os.path.join(args.output_dir, "OmniMedVQA"), exist_ok=True)
    
    # Setup logging filename
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_filename = os.path.join(args.output_dir, f"OmniMedVQA/omnimedvqa_{args.model_variant}_{timestamp}.json")
    live_log_filename = os.path.join(args.output_dir, f"OmniMedVQA/omnimedvqa_{args.model_variant}_{timestamp}_live.json")
    ...
    image_folder = os.path.join(args.data_dir, "")
    ...
    # Process each sample
    for sample in tqdm(test_set, desc="Processing OmniMedVQA"):
...
Base your answer only on the provided images and case information. Only respond with your SINGLE LETTER answer: """
        ...
        # Get image path
        image_path_rel = sample['image_path']
        if isinstance(image_path_rel, str):
            image_path = os.path.join(image_folder, image_path_rel)
        elif isinstance(image_path_rel, list):
            image_path = os.path.join(image_folder, image_path_rel[0])
        else:
    ...
    # Print summary
    logging.info("\n" + "="*60)
    logging.info("OmniMedVQA Benchmark Results")
    logging.info("="*60)
