...
def create_inference_request(
    ...
    # figures
    question_fig = sample['image_path']
...
Base your answer only on the provided images and case information. Respond with your SINGLE LETTER answer: """
    ...
    try:
        ...
        if isinstance(question_fig, str):
            image_paths = [image_folder + question_fig]
        elif isinstance(question_fig, list):
            image_paths = [f"{image_folder}{p}" for p in question_fig]
        else:
            image_paths = [str(question_fig)]
    ...
    try:
        ...
        if len(processed_images)>0:    
            ...
            # message = (text, processed_images[0], "Pad")  # Use Pad mode to expand image to square
        else:
            print(f"No local images found!")
            return "skipped", 0.0  # Return a special 'skipped' marker
...
def main():
    ...
    parser.add_argument("--data-dir",type=str,default="../../../data/OmniMedVQA/", help="Path to OmniMedVQA data directory")
    parser.add_argument(
    ...
    os.makedirs(os.path.join(args.output_dir, "Omni"), exist_ok=True)
    live_log_filename = os.path.join(args.output_dir, f"Omni/Omni_live_benchmark_log_{timestamp}.json")
    final_results_filename = os.path.join(args.output_dir, f"Omni/Omni_final_results_{timestamp}.json")
    ...
    # Load dataset
    # path = './OmniMedVQA/sampled_vqa_500.json'
    path = os.path.join(args.data_dir, 'sampled_vqa_500.json')
    test_set = []
    try:
    ...
    # Process each case
    for sample in tqdm(test_set, desc="Processing cases"):
        ...
        # Handle error cases (when function returns None, None)
        if inference_result == (None, None) or inference_result is None:
            skipped_questions += 1
            print(f"\nQuestion {question_id}: Failed (Error in inference)")
            ...
            with open(live_log_filename, "a") as live_log_file:
                json.dump(error_entry, live_log_file, indent=2)
                live_log_file.write(",\n")  # Add comma for next entry
        ...
        log_entry = {
            ...
            'question_ture_id': sample['question_id'],
            'question_type': sample['question_type'],
            'modality_type': modality_type,
            "question": sample["question"],
            "correct_answer": sample["gt_answer"],
            
            "raw_output": inference_result["raw_output"],
            "validated_answer": answer,
            "model_answer": answer,
            "is_correct": answer == sample["gt_answer"] if answer else False,
            "duration": duration,
        ...
        print(f"Correct Answer: {sample['gt_answer']}")
        print(f"Time taken: {duration:.2f}s")

        # Track correct answers
        if answer == sample["gt_answer"]:
            correct_answers += 1
