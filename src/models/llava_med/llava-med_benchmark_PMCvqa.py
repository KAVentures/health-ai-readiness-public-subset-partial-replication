...
def create_inference_request(
    ...
    # image folder path
    image_folder = os.path.join(data_dir, "figures")
    
    # figures
    question_fig = sample['Figure_path']
...
def main():
    ...
    os.makedirs(os.path.join(args.output_dir, "PMC"), exist_ok=True)
    live_log_filename = os.path.join(args.output_dir, f"PMC/PMC_live_benchmark_log_{timestamp}.json")
    final_results_filename = os.path.join(args.output_dir, f"PMC/PMC_final_results_{timestamp}.json")
    ...
    # Load dataset
    path = './PMC-VQA/test_2_500_sample_filter.json'
    ...
    for sample in tqdm(test_set, desc="Processing cases"):
        ...
        log_entry = {
            ...
            "model_answer": answer,
            "is_correct": answer == sample["Answer"] if answer else False,
            "duration": duration,
