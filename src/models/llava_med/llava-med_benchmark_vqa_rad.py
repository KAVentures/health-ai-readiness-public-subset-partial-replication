...
def create_inference_request(
    ...
    """
    system_prompt = """You are a radiology expert.
    """
    ...
    # figures
    question_fig = sample['image']
   
    prompt = f"""Please analyze the following medical image and answer the question based on the image and your medical knowledge.
    ...
    try:
        ...
        if isinstance(question_fig, str):
            image_paths = [f"{image_folder}{sample['image_folder']}/{sample['image']}"]
...
def main():
    ...
    parser.add_argument("--data-dir",type=str,default="../../../data/VQA_RAD/", help="Path to VQA_RAD data directory")
    parser.add_argument(
    ...
    os.makedirs(os.path.join(args.output_dir, "VQARAD"), exist_ok=True)
    live_log_filename = os.path.join(args.output_dir, f"VQARAD/VQARAD_live_benchmark_log_{timestamp}.json")
    final_results_filename = os.path.join(args.output_dir, f"VQARAD/VQARAD_final_results_{timestamp}.json")
    ...
    # Load dataset
    # path = '/home/jifu/Medagent/VQA_RAD/test.json'
    ...
    for sample in tqdm(test_set, desc="Processing cases"):
        ...
        if inference_result == ("skipped", 0.0):
            skipped_questions += 1
            print(f"\nQuestion {question_id}: Skipped (No images)")
