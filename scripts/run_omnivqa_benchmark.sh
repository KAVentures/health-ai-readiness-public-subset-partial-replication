...
# Examples:
#     python test_runner_async.py OmniVQA gpt-4o t0 vqa zero
#     python test_runner_async.py OmniVQA o3 t1 vqa high
#     python test_runner_async.py OmniVQA deepseek t0 vqa zero
#     python test_runner_async.py OmniVQA gemini-2.0-flash t0 vqa zero
#     python test_runner_async.py OmniVQA claude-sonnet-3.5 t0 vqa zero
#     python test_runner_async.py OmniVQA gpt-4o t0 text zero

# Async test script for OmniVQA dataset
    ...
    for text_flag in "${TEXT_FLAGS[@]}"; do
        ...
        for run in $(seq 0 $((NUM_RUNS - 1))); do
            ...
            if [ $exit_code -ne 0 ]; then
                echo "WARNING: Test failed with exit code $exit_code"
